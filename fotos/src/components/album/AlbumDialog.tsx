  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEditMode && albumToEdit?.id) {
        await axiosInstance.put(`/api/albums/${albumToEdit.id}`, formData )
        emitAlbumRefresh();
        toast.success("Album updated successfully.");
      } else {
        await axiosInstance.post("/api/albums", formData);
        emitAlbumRefresh();
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : (error as any).response?.data?.error || "An error occurred while saving the album";
